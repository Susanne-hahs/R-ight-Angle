...
shift.p_long <- shift.p %>%
  pivot_longer(cols = c("control", "fatigue"), names_to = "activity") %>%
  print()

shift.p_long %>%
  ggplot(aes(x=who, y=value, fill=activity)) + geom_bar(stat = "identity", position = "dodge", width = 0.5) + scale_x_discrete(labels = NULL) + theme_classic() + coord_flip()
